    ...
    Chatbox.prototype = {
        ...
        init: function (options){
            ...
            switch (opts.type){
            	...
            	case 'system':
            		boxFrame = '\
                        <div class="systembox" id="'+opts.boxId+'">\n\
                            <div class="chatbox-header">\n\
                                <div class="chatbox-title">'+opts.title+'</div>\n\
                                <div class="chatbox-options">\n\
                                    <a class="minimize" href="#">-</a>\n\
                            ...
                            <div class="chatbox-body">\n\
                                <div class="systembox-content"></div>\n\
                            </div>\n\
                        </div>\n';
            		break;
            	default:
            ...
            if (msg == ''){
                ...
                var msgItem = '\
                    <div class="chatbox-message">\n\
                        <span class="message-by">'+this.opts.user+'：</span>\n\
        ...
        message: function(msg,type){
            ...
            switch (type){
                ...
                case 'system':
                    ...
                    this.$elem.find('.systembox-content').append(msgItem);
                    this.$elem.find('.systembox-content').scrollTop(this.$elem.find('.systembox-content').get(0).scrollHeight);
                    this.blink();
                    this.animate();
        ...
        ,
        // 自动销毁
        aotuDestroy: function(){
            var self = this;
            // 从页面淡出该DOM元素并从页面移除
            this.$elem.fadeOut(2000,function(){
                $(this).remove();
                // 销毁对象实例
                boxInstance[self.opts.id] = null;
                delete boxInstance[self.opts.id];
